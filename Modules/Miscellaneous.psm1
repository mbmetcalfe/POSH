...
$UptimeConfigFile = ([Environment]::GetFolderPath("MyDocuments") + "\UpTime.xml")
...
function Get-Uptime
{
    ...
    Write-Verbose ("Configuration file: {0}." -f ($UptimeConfigFile))

    if (!(Test-Path $UptimeConfigFile))
    {
        # Create The Document  
        $XmlWriter = New-Object System.XMl.XmlTextWriter($UptimeConfigFile, $Null)  
        # Set The Formatting  
    ...
    [xml]$ConfigFile = Get-Content $UptimeConfigFile
    $uptimeSettings = @{
    ...
    $ConfigFile.Save($UptimeConfigFile)
}
New-Alias -Name uptime -Value Get-Uptime -Description "Check and/or display the current machine's up time."
